...
const ListM = () => {
  return (
    <IMG imgSrc="./images/list_mark.gif" imgAlt="" imgTitle="" />
  )
}

const Favicon = ({siteUrl}) => {
  const googleFavicon = `http://www.google.com/s2/favicons?domain=${siteUrl}`
  return <img src={googleFavicon} className="iconyou" />
}
...
const Subcontent = () => (
  <div className="global-Subcontent">
    ...
    <p><ListM />{Link("http://opac.city.urayasu.chiba.jp/opw/OPW/OPWSRCH1.CSP","本を探す")}</p>
      ...
      <p><ListM />{Link("https://opac.city.urayasu.chiba.jp/opw/OPW/OPWNEWBOOK.CSP","新着資料")}</p>
      <p><ListM />{Link("https://opac.city.urayasu.chiba.jp/opw/OPW/OPWLOGINTIME.CSP?HPFLG=1&NEXT=OPWUSERINFO&DB=LIB","Ｍｙページ（利用状況確認）")}</p>
      <p><ListM />{Link("https://opac.city.urayasu.chiba.jp/opw/OPS/OPSINDEX.CSP","スマートフォン対応検索画面")}</p>
    </div>
